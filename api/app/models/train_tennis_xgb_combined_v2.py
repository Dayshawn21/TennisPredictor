# FILE: api/app/models/train_tennis_xgb_combined_v2.py
"""
Train a combined XGBoost model using simple rolling + TA rolling features.
Uses the same time-based split and calibration logic as the simple v2 trainer.
"""
...
DEFAULT_CSV_PATH = PROJECT_ROOT / "tennis_training_combined.csv"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train combined v2 tennis XGB model.")
    parser.add_argument("--csv", type=str, default=str(DEFAULT_CSV_PATH))
    parser.add_argument("--model-out", type=str, default=str(MODEL_DIR / "xgb_tennis_combined_v2.json"))
    parser.add_argument("--features-out", type=str, default=str(MODEL_DIR / "xgb_tennis_combined_v2_features.txt"))
    parser.add_argument("--metadata-out", type=str, default=str(MODEL_DIR / "xgb_tennis_combined_v2_meta.json"))
    parser.add_argument("--calibration-out", type=str, default=str(MODEL_DIR / "xgb_tennis_combined_v2_calibration.joblib"))
...
def main() -> None:
    ...
    X_train = fit_df[FEATURE_COLS].copy().fillna(0.0)
